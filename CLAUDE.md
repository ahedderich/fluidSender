...
- **Known Release Please quirk:** `googleapis/release-please-action`'s `target-branch` **workflow input** — not a `target-branch` key in `.release-please-config.json`, which isn't a real schema field and is silently ignored — is what actually controls which branch it opens/merges the release PR against. If that input is left unset, the action auto-detects it from the **repository's GitHub default branch**, not from the `on: push: branches:` filter that triggered the run. `release-please.yaml` pins this explicitly (`target-branch: main`) so release automation can't silently start targeting the wrong branch if the repo's default branch is ever changed away from `main` (e.g. to `test`) — don't remove that input.
