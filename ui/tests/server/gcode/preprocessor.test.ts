...
import type { MachineKinematics } from '../../../server/utils/gcode/kinematics'

const RAPID = 3000 // mm/min

// Very large acceleration + junction deviation so ramp time and cornering slowdown are
// both negligible — degenerates to the old pure distance/feedrate arithmetic these tests
// were written against, so none of their expected values need to change.
const INSTANT_KINEMATICS: MachineKinematics = {
  x: { maxRateMmPerMin: RAPID, accelerationMmPerSec2: 1e9 },
  y: { maxRateMmPerMin: RAPID, accelerationMmPerSec2: 1e9 },
  z: { maxRateMmPerMin: RAPID, accelerationMmPerSec2: 1e9 },
  junctionDeviationMm: 1e6,
}
...
describe('preprocessGCode', () => {
  ...
  it('accumulates cumulative duration correctly', () => {
    const gcode = 'G0 X10\nG1 X20 F600\n'
    const { lines } = preprocessGCode(gcode, INSTANT_KINEMATICS)
    closeTo(
  ...
  it('computes axis ranges', () => {
    ...
    const { axisRanges } = preprocessGCode(gcode, INSTANT_KINEMATICS)
  ...
  it('tracks position through modal G1 lines (no G word)', () => {
    ...
    const { lines, vectors } = preprocessGCode(gcode, INSTANT_KINEMATICS)
  ...
  it('arc vectors connect to preceding motion', () => {
    ...
    const { vectors } = preprocessGCode(gcode, INSTANT_KINEMATICS)
