  ...
  <main class="flex-1 flex min-h-0 overflow-hidden">
    ...
    <section class="flex-1 overflow-y-auto">
      ...
      <div v-else-if="editingMachine" class="p-5 space-y-4 max-w-3xl">
        ...
        <!-- Tab: Tool Change -->
        <template v-else-if="machineTab === 'toolchange'">
          ...
          <template v-else-if="tc.strategy === 'manual-toolsetter'">
            ...
            <SettingsCard title="Probe Settings">
              <SettingsRow label="Wiggle Probing">
                <ToggleSwitch v-model="(tc as any).position.probeConfig.wiggleEnabled" />
              </SettingsRow>
            ...
            <SettingsCard title="Offset Adjustment">
              ...
              <SettingsRow label="Confirm After Probe">
                <ToggleSwitch v-model="(tc as any).position.confirmAfterProbe" />
          ...
          <template v-else-if="tc.strategy === 'atc-passthrough'">
            ...
            <SettingsCard title="Tool Magazine">
              <SettingsRow label="Enable Magazine">
                <ToggleSwitch v-model="(tc as any).magazine.enabled" />
              </SettingsRow>
...
// ─── Toolchange tab helpers ───────────────────────────────────────────────────

const tc = computed<ToolchangeConfig>(() => {
  return (editingMachine.value?.toolchange as ToolchangeConfig | undefined) ?? { strategy: 'manual-basic' }
})
