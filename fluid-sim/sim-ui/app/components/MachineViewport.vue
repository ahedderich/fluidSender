      ...
      <div class="flex-1 flex flex-col gap-1 min-w-0 min-h-0">
        ...
        <div class="flex-1 bg-gray-50 dark:bg-slate-900 rounded border border-gray-200 dark:border-slate-700 overflow-hidden min-h-0">
          ...
          >
            ...
            <rect x="0" y="0" :width="s.travel.x" :height="s.travel.y"
            ...
            <g opacity="0.6">
              ...
              <line v-for="y in gridY" :key="`gy${y}`" x1="0" :y1="y" :x2="s.travel.x" :y2="y" stroke="#e5e7eb" stroke-width="0.3" />
            </g>

            <!-- Stock body -->
            <rect v-if="s.stock.shape === 'rect'"
              :x="s.stock.ox" :y="s.stock.oy"
            ...
            <circle v-else
              :cx="s.stock.ox + s.stock.diameter / 2" :cy="s.stock.oy + s.stock.diameter / 2"
              :r="s.stock.diameter / 2"
              fill="#bfdbfe" fill-opacity="0.7" stroke="#3b82f6" stroke-width="0.8" />

            <!-- Hole (rendered over stock body as a cutout) -->
