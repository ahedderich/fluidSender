...
impl StockDefinition {
    ...
    /// Test whether a point (machine coords) is inside the stock volume.
    pub fn contains_point(&self, p: [f64; 3]) -> bool {
        let [x, y, z] = p;
        z <= self.top_z() + 0.001
            && z >= self.bottom_z() - 0.001
...
mod tests {
    ...
    fn contains_point_inside() {
        let s = rect_stock();
        assert!(s.contains_point([100.0, 60.0, 0.0]));
    }

    #[test]
    fn contains_point_above() {
        let s = rect_stock();
        assert!(!s.contains_point([100.0, 60.0, 5.1]));
    }

    #[test]
    fn contains_point_below() {
        let s = rect_stock();
        assert!(!s.contains_point([100.0, 60.0, -15.1]));
    }

    #[test]
    fn contains_point_outside_xy() {
        let s = rect_stock();
        assert!(!s.contains_point([5.0, 5.0, 0.0]));
    }

    #[test]
    fn contains_point_in_hole() {
        let mut s = rect_stock();
        s.hole = Some(HoleDefinition {
            x: 0.0,
            y: 0.0,
        ...
        assert!(!s.contains_point([100.0, 60.0, 0.0]));
        assert!(s.contains_point([120.0, 60.0, 0.0]));
    }
}
